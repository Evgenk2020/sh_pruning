...
echo 'set(CMAKE_C_STANDARD 17)' >> $CMK
echo 'set(CMAKE_C_STANDARD_REQUIRED ON)' >> $CMK
...
echo "" >> $CMK
echo 'project('$PROJECT_NAME' LANGUAGES C)' >> $CMK
echo "" >> $CMK
echo 'set(SRC src/main.c)' >> $CMK
echo "" >> $CMK
echo '#add_library(one STATIC lib/*.h lib/*.c)' >> $CMK
echo '#add_library(two SHARED lib/*.h lib/*.c)' >> $CMK
echo "" >> $CMK
...
echo '{' >> $C_FILE
echo '  printf("Project name: '$PROJECT_NAME'\n");' >> $C_FILE
